"""
Unit tests for the scoring engine (Stage C), including:
- qualification eligibility
- field-of-study matching
- CGPA normalization
- budget eligibility
- seats eligibility
- English score eligibility
- weighted scoring
- ranking
"""
...
def test_exact_field_of_study_match():
    student = make_student(field_of_study="Computer Science")
    program = make_program(field_of_study="Computer Science")
...
def test_partial_match_scores_lower_than_full_match():
    ...
    program_partial = make_program(
        id="uni-2",
        country="Germany",
    )

    full_result = score_student_against_program(
        student,
        program_full,
    )

    partial_result = score_student_against_program(
        student,
        program_partial,
    )
...
def test_marks_ratio_is_capped():
    ...
    program = make_program(
        min_marks_percentage_cutoff=50.0,
    )

    result = score_student_against_program(
        student,
        program,
    )
    ...
    assert result.score <= max_possible_score + 0.0001
